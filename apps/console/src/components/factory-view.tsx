...
export function FactoryView({ embedded = false }: { embedded?: boolean }) {
    ...
    <div className="space-y-8">
      ...
      <SlideUp delay={0.02}>
        <CommandPanel
          ...
          description="Track in-flight deals and open each engagement’s factory runbook for CLI steps, env checks, and deploy hooks."
        >
          <div className="flex flex-wrap gap-2 text-sm">
            <Button variant="outline" size="sm" asChild>
              <Link href="/configure?tab=playbooks">
                Delivery playbooks
      ...
      <SlideUp delay={0.04}>
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <Button variant="ghost" size="sm" className="text-muted-foreground" asChild>
            <Link href="/pipeline?stage=delivery">All deals</Link>
          </Button>
        </div>
        {boardQ.isLoading ? (
          ...
          <EmptyState
            title="No preset deals in flight"
            description="File a deal from the launch wizard — matching economics appear here automatically."
            action={
              <Button asChild>
                <Link href="/build?tab=launch">Open launch wizard</Link>
              </Button>
            }
          />
        ) : (
          <CommandPanel title="Active delivery">
      ...
      <SlideUp delay={0.06}>
        ...
        <p className="mb-4 max-w-3xl text-sm text-muted-foreground">
          Canonical economics for clone and template paths. Custom scopes still start in the launch wizard or new deal
          form.
        </p>
      </SlideUp>
