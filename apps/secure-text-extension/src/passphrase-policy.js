...
(function (global) {
  ...
  function assessPassphrase(passphrase, profile = 'personal', { mode } = {}) {
    ...
    if (WEAK_EXACT.has(lower)) {
      return { ok: false, score: 1, message: 'This passphrase is too common. Use a random team secret from 1Password.', minLength };
    }
    ...
    if (needsDiversity && diversity < 2) {
      return {
        ok: false,
        score,
        message: 'Mix upper, lower, digits, or symbols — or use a generated passphrase from 1Password.',
        minLength,
      };
    }
